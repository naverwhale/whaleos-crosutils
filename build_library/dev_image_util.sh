...
install_dev_packages() {
  ...
  echo "Modifying Release Track for Dev (devimage-channel)."
  FILE="${root_fs_dir}/etc/lsb-release"
  sudo sed -i 's/^\(CHROMEOS_RELEASE_TRACK=\).*/\1devimage-channel/' $FILE
  sudo -E ROOT_FS_DIR="${root_fs_dir}" \
    "${SCRIPTS_DIR}/mod_for_test_scripts/300changePassword"
  sudo -E ROOT_FS_DIR="${root_fs_dir}" \
    "${SCRIPTS_DIR}/mod_for_test_scripts/350addEnvironment"
  sudo -E ROOT_FS_DIR="${root_fs_dir}" \
    "${SCRIPTS_DIR}/mod_for_test_scripts/370fixTestSSH"
  sudo -E ROOT_FS_DIR="${root_fs_dir}" BUILD_DIR="${BUILD_DIR}" \
    "${SCRIPTS_DIR}/mod_for_test_scripts/380archiveTestKeys"
