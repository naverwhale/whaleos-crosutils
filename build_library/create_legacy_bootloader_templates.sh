...
verity_common="${verity_common} quiet"
...
menuentry "Previous Successful Boot" {
  linux /syslinux/vmlinuz.A ${common_args} ${verity_common} \
      i915.modeset=1 cros_efi root=${ROOTDEV} dm="DMTABLEA"
}

menuentry "Load System Recovery Menu" {
  linux /syslinux/vmlinuz.minios ${common_args} i915.modeset=1 cros_efi cros_minios \
      root=/dev/\$linuxpartA
}
EOF
  if [[ ${FLAGS_enable_rootfs_verification} -eq ${FLAGS_TRUE} ]]; then
